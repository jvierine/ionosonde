...
def overview_plots(t0, t1, max_r=500, gc=20):
    ...
    for fi, f in enumerate(idx):
        ...
        for freq_i in range(I.shape[0]):
            I[freq_i, :]=I[freq_i, :]/np.median(np.abs(I[freq_i, :]))
        # plt.pcolormesh(I)
        # plt.colorbar()
        # plt.show()
        # plt.pcolormesh(I)
        # plt.show()
        # print(I.shape)
        OR[fi, :]=np.max(I[:, 0:max_r], axis=0)
        OF[fi, :]=np.max(I[:, gc:max_r], axis=1)

    dBOR=10.0*np.log10(OR)
    dBOF=10.0*np.log10(OF)
    dBOF[np.isinf(dBOF)]=np.nan
    dBOR=dBOR-np.nanmedian(dBOR)
    dBOF=dBOF-np.nanmedian(dBOF)
    dBOF[np.isnan(dBOF)]=-3
    t=np.array(t)
    plt.figure(figsize=(1.5*8, 1.5*6))
    # 30 km range shift
    plt.pcolormesh(dtt, rvec-iono_config.range_shift*1.5, np.transpose(dBOR), vmin=0, vmax=10)
    plt.colorbar()
    plt.xlabel("Time (UTC)")
...
#    plt.show()
    ...
    plt.figure(figsize=(1.5*8, 1.5*6))
    plt.pcolormesh(dtt, 0.5*(freq[:, 0]+freq[:, 1]), np.transpose(dBOF), vmin=0, vmax=10)
    plt.colorbar()
    plt.xlabel("Time (UTC)")
